...
namespace FilteredLookupFields
{
    /// <summary>
    ...
    public sealed class FilteredLookupFieldControl : LookupField
    {
        ...
        private void CreateStandardSelect()
        {
            ...
            if (!Field.Required) { l.Items.Insert(0, new ListItem("(Ninguno)", "0")); }
            this.Controls.Add(l);
        }
        #endregion
        ...
        private string ConcatAvailableItems(string delimiter)
        {
            ...
            if (!Util.ListIsNullOrEmpty(_availableItems))
            {
                if (!this.Field.Required) { retval += string.Format(CultureInfo.InvariantCulture, "{0}{1}{0}0", delimiter, "(Ninguno)"); }
            ...
            get
            {
                ...
                if (c != null)
                {
                    if (c is System.Web.UI.WebControls.DropDownList)
                    {
                        ...
                        if (ctrl.SelectedItem.Value != "0" && ctrl.SelectedItem.Text != "(Ninguno)")
                        {
